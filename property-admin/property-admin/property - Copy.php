...
if($_GET['action']=="search") { 
	...
	if( !isset($_POST['externalid']) ) { echo "<br /><p id='search_msg'>Enter 'Stock Number' and click 'Find Property' button to search.</p>"; } else {
		if( mysql_num_rows($result) > 0 ) {
			include('admin_display.inc.php');
			echo "<br/><p>Use respective tab pages to manage this property information.</p>";
...
<?	
	...
	if( isset($_SESSION['addMSG']) ) { echo $_SESSION['addMSG']; unset($_SESSION['addMSG']); } else {
		...
		//echo "<div align='center'><h2>Use following form to add  a new property</h2></div>";
		//if(isset($_POST['externalid'])) { echo '<form id="add_school_form" name="add_school_form" method="post" action="admin_add_school.php">'; }
		echo '<form id="add_property_form" name="add_school_form" method="post" action="admin_process_property.php">'; 
		include('admin_display.inc.php');	
		echo '<br /><br /><div align="center"><input type="reset" class="btn_style" />&nbsp;&nbsp;<input type="submit" value="Add Property" class="btn_style" /></form></div>';
	}
	...
	if( isset($_SESSION['addMSG']) ) { echo $_SESSION['addMSG']; unset($_SESSION['addMSG']); } else {
		...
		if( !isset($_POST['externalid']) ) { echo "<br /><p id='search_msg'>Enter 'Stock Number' and click 'Find Property' button to search.</p>"; } else {
			if( mysql_num_rows($result) > 0 ) {
				echo '<form id="edit_property_form" name="edit_school_form" method="post" action="admin_process_property.php">'; 
				include('admin_display.inc.php');
	...
	if( isset($_SESSION['addMSG']) ) { echo $_SESSION['addMSG']; unset($_SESSION['addMSG']); } else {
		...
		if( !isset($_POST['externalid']) ) { echo "<br /><p id='search_msg'>Enter 'Stock Number' and click 'Find Property' button to search.</p>"; } else {
			if( mysql_num_rows($result) > 0 ) {
				echo '<form id="delete_property_form" name="delete_school_form" method="post" action="admin_process_property.php">'; 
				include('admin_display.inc.php');
				echo '<br /><br /><div align="center"><input type="submit" value="Delete Property" class="btn_style" /></form></div>';	
			} else { echo '<br/><br/><br/><div align="center"><img src="../images/error.jpg" /><br/>No records found!</div>'; }
		}
	}
}
